    ...
    <div className="mx-auto w-full max-w-[1464px]">
      ...
      <div className="mb-3.5 flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={() => navigate('/qa')}
          className="h-10.5 w-55 shrink-0 rounded-[10px] bg-charcoal text-sm font-bold text-milk"
        >
          {t('featureDetail.ask')}
        </button>
      </div>
      ...
      <section className="mb-2 rounded-[10px] bg-almond-milk px-5 pt-5 pb-6">
        <div className="mb-2.5 flex items-center gap-2.5">
          <GithubIcon className="h-[24px] w-[24px] shrink-0 text-dark-lava" />
          <h2 className="text-xl font-bold text-dark-lava">{feature.title[lang]}</h2>

          {/* 진행도 + 상태 */}
          <div className="mr-[27px] ml-auto flex h-12.5 w-50 items-center justify-center gap-3 rounded-[8px] bg-milk">
            <span className="text-lg font-semibold text-dark-lava">{feature.progress}%</span>
            <span className="h-6 w-0.5 rounded-full bg-mocha" />
            <StatusBadge status={feature.status} />
          </div>
        </div>

        <p className="mx-[27px] whitespace-pre-line rounded-[8px] bg-milk px-6 py-3.5 text-sm font-medium leading-relaxed text-taupe">
          ...
          {feature.linked.map((item) => (
            ...
            >
              <div className="min-w-0 flex-1">
                <p className="mb-1 truncate text-lg font-semibold text-dark-lava">{item.title[lang]}</p>
                <p className="text-sm font-bold text-mocha">{item.meta[lang]}</p>
              </div>
              <a
                href={item.url}
                className="shrink-0 text-sm font-bold text-taupe underline underline-offset-4 hover:text-dark-lava"
              >
      ...
      <section className="rounded-[10px] bg-almond-milk px-5 pt-5 pb-6">
        <div className="mb-2.5 flex items-center gap-2.5">
          <CheckCircleIcon className="h-6 w-6 shrink-0 text-dark-lava" />
          <h2 className="text-xl font-bold text-dark-lava">{t('featureDetail.evidence')}</h2>
        </div>

        <p className="mx-[27px] whitespace-pre-line rounded-[8px] bg-milk px-6 py-4 text-sm font-medium leading-relaxed text-taupe">
