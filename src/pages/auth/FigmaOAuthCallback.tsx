// Figma 소스 연동 콜백 페이지
// 서버가 Figma 인가를 처리한 뒤 ?workspaceId=&status=success|error 로 이 경로로 리다이렉트
// Figma는 Github와 달리 선택 가능한 목록이 없어, 사용자가 파일 링크를 직접 입력

import { useEffect, useRef, useState } from 'react'
import { useNavigate, useSearchParams } from 'react-router-dom'
import { useTranslation } from 'react-i18next'
import { motion } from 'framer-motion'
import { FigmaIcon, ErrorIcon, ReloadIcon } from '../../components/ui/icons/ModalIcons'
...
function FigmaOAuthCallback() {
  ...
  const confirmLink = () => {
    ...
    if (!value) {
      setErrorMessage(t('figmaCallback.linkRequired'))
      return
    }

    setPhase('saving')
    setErrorMessage(null)
    createSourceConnection(workspaceId, { sourceType: 'FIGMA', targetRepoOrBoard: value })
      .then(() => {
        /* 연동을 시작했던 화면으로 복귀 */
      ...
      .catch((e) => {
        setErrorMessage(e instanceof ApiError ? e.message : t('figmaCallback.saveFailed'))
        setPhase('entering')
      })
  }
  ...
  const retry = () => {
    ...
    startSourceOAuth('figma', workspaceId, getOAuthReturnPath() ?? DEFAULT_RETURN_PATH)
  }
      ...
      >
        ...
        {phase === 'loading' && (
          <div className="flex flex-col items-center gap-4 py-10 text-center">
            <FigmaIcon className="h-14 w-14 text-mocha" />
            <p className="text-lg font-semibold text-mocha">{t('figmaCallback.loading')}</p>
          </div>
        )}
        ...
        {phase === 'invalid' && (
          <div className="flex flex-col items-center gap-4 py-6 text-center">
            ...
            <div>
              <p className="mb-1 text-xl font-semibold text-dark-lava">{t('figmaCallback.invalidTitle')}</p>
              <p className="text-base text-taupe">{t('figmaCallback.invalidDesc')}</p>
            </div>
            <button
              type="button"
              onClick={goBack}
            ...
            >
              {t('figmaCallback.back')}
            </button>
          </div>
        )}

        {/* Figma 인증 자체가 실패한 경우 (status=error) */}
        {phase === 'error' && (
          <div className="flex flex-col items-center gap-4 py-6 text-center">
            <ErrorIcon className="h-14 w-14 text-mocha" />
            <div>
              <p className="mb-1 text-xl font-semibold text-dark-lava">{t('figmaCallback.errorTitle')}</p>
              <p className="text-base text-taupe">{errorMessage ?? t('figmaCallback.errorDesc')}</p>
            </div>
        ...
        {(phase === 'entering' || phase === 'saving') && (
          <div>
            <div className="mb-5 flex items-center gap-4">
              <FigmaIcon className="h-10 w-10 text-mocha" />
              <div>
                <p className="text-xl font-semibold text-dark-lava">{t('figmaCallback.inputHeading')}</p>
                <p className="text-sm text-taupe">{t('figmaCallback.inputDesc')}</p>
              </div>
            </div>
            ...
            <input
              ...
              placeholder={t('figmaCallback.placeholder')}
              aria-label={t('figmaCallback.inputHeading')}
            ...
            <div className="mt-3 flex gap-3">
              ...
              >
                {phase === 'saving' ? t('figmaCallback.saving') : t('figmaCallback.confirm')}
              </button>
              <button
                type="button"
                onClick={goBack}
...
export default FigmaOAuthCallback